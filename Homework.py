...
print('Задание №4 ↓')
number = int(input('Введите целое положительное число, программа определит наибольшую цифру: '))
max_number = 0
while number != 0:
...
#_____________________ Задание 5/6 (Оценка прибильности компании\ рассчёт доходности на человека)
print('Задание №5/6 ↓')
earnings, costs = float(input('Введите сумму доходов компании: ')), float(input('Введите сумму затрат компании: '))
if earnings > costs:
    profit = earnings - costs
    print(f'Компания приносит доход! Чистая прибыль составила {profit} у.е.'
          f'Рентабельность составляет: {round((profit / earnings) * 100, 1)} %')
    print('Прибыль компании в расчёте на одного сотрудника = ',
          round(profit / int(input('Введите количество сотрудников: ')), 2))
elif costs > earnings:
...
#_____________________ Задание 7 (Считаем количество дней для достижения спортсменом результата)
print('Задание №7 ↓')
first_result = int(input('Введите количество километров в первый день: '))
need_result = int(input('Введите желаемый результат в км: '))
days = 1

while first_result < need_result:
    first_result *= 1.1
    days += 1
print(f'На {days} -й день спортсмен достигнет результата — не менее {need_result} км. ')